...
const registrar = async (req, res) => {
    ...
    if (existeUsuario) {
        ...
        return res.status(400).json({ msg: error.message }) //para cambiar el código de error
    }

    try {
        //Guardar un Nuevo Veterinario
        const veterinario = new Veterinario(req.body);
        const veterinarioGuardado = await veterinario.save();//.save() --> metodo para guardar en la base de datos
...
const confirmar = async (req, res) => {
    const { token } = req.params; //Cuando lees datos de la URL
